...
#Hence, there are reps streams on every core, each using one seed. The problem of repeated seeds is solved in this way:
...
#This should be ok, but I have not checked whether that's really reps*num_cores identical unique seeds.
#Solved: Recalculated seeds such that they must be unique:
#2*(myseed*(reps+1)+rep_sim) and 2*(myseed*(reps+1)+rep_sim)-1 obviously produce unique seeds I added and multiplied different constants on them.
#Probably not necessary, but "masks" pattern in seeds a bit.
