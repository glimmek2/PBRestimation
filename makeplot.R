...
plot(stepfun(diffcurve$stoptime,c(0,diffcurve$p1Diff)),do.points=FALSE,col='darkgrey',
     xlim=c(0,max(diffcurve$stoptime)+100),ylim=c(ylimlow-0.05,ylimup+0.05), main='difference of probability of being in response',
     xlab='days',ylab='probability')
lines(stepfun(diffcurve$stoptime,c(0,diffcurve$lowlimDiff1)),do.points=FALSE, col='darkgrey',lty='dotted')
lines(stepfun(diffcurve$stoptime,c(0,diffcurve$uplimDiff1)),do.points=FALSE, col='darkgrey',lty='dotted')
