#Functions needed to simulate the data for Glimm and Hollaender (2025): Testing probability of being in response.
...
simulate1trialarm<-function(myseed,scenario){
  ...
  simTimes$time01obs<-ifelse(simTimes$cens01==FALSE, simTimes$time0x,
    ...
    #time 0->2 if 0->2 observed. In this case cens01=TRUE, so time 0->2 is marked as "censored".
  ...
  simTimes$time02obs<-ifelse(simTimes$cens02==FALSE, simTimes$time0x,
    ...
    #time 0->1 if 0->1 observed. In this case cens02=TRUE, so time 0->1 is marked as "censored".
    #time censored if patient censored in state 0
